...
def Trimmomatic_parser():
	...
	parser = argparse.ArgumentParser( 
		...
		description='A python program that launch Trimmomatic Tool : A Adapter \
and Quality Trimming Tools for high throughput sequencing data',
		epilog="\nSome examples:\n\n\t\
'python Filtrage.py -h' \t for help\n\t\
'python Filtrage.py --XML' \t to launch trimmomatic using parameters from the \
XML file\n\t\
'python Filtrage.py PE read_1.fastq read_2.fastq -illuminaclip \
fasta-file.fa:2:10:30' \t for adapter trimming\n\t\
'python Filtrage.py SE read_1.fq -slidingwindow 10:30 -leading 30 -minlen 36' \
\t for quality trimming\n\t\
'python Filtrage.py PE read_1.fq.bz2 read_2.fq.bz2 -illuminaclip \
fasta-file.fa:2:10:30 -slidingwindow 10:30 -minlen 36' \t for adapter and \
quality trimming.")
...
'readfile.fastq' || 'readfile.fq.gz'\n  For PE 'read_1.fastq read_2.fastq' || \
...
<fastaWithAdaptersEtc>:<seed mismatches>:<palindrome clip threshold>:<simple \
...
<window-size>:<required-quality>\n  Recommended: 4:30 or 10:30')
	
	parser.add_argument("-maxinfo",
						type=str,
						action='store',
						help='Adaptative quality trimming.\n  Usage: -maxinfo \
<target-length>:<strictness>')
	
	parser.add_argument("-leading",
						type=int,
						action='store',
						help="minimum quality required to keep a base in 5' \
end.\n  Usage: '-leading <required-quality>'")
	
	parser.add_argument("-trailing",
						type=int,
						action='store',
						help="minimum quality required to keep a base in 3' \
end.\n  Usage: '-trailing <required-quality>'")
	
	parser.add_argument("-crop",
						type=int,
						action='store',
						help="number of base to remove from the end of the \
read.\n  Usage: '-crop <number>'")
	
	parser.add_argument("-headcrop",
						type=int,
						action='store',
						help="number of base to remove from the start of the \
read.\n  Usage: '-head <number>'")
 ...
 Usage: '-minlen <length>'")
	...
	parser.add_argument("-tophred33",
						action='store_const',
						const='TOPHRED33',
						help="(re)encodes the quality part of the FASTQ file to\
 base 33.\n  Usage: '-tophred33'")
...
def check_layout(arg):
	"""
	Function that check that a layout have been entered and it's either 'SE'
	(Single-Ends) or 'PE'(Paired-Ends).
	...
	if(layout != None):
		...
		if(layout == 'PE' or layout == 'SE'):
			...
			sys.exit("Error : Layout can only be 'SE' (for single-end) or 'PE'\
 (for paired-ends).")
...
def check_input(arg):
	...
	Returns:
		...
		- 2 [integer] : if input files extension is '.fastq/fq.bz2' 
						or '.fastq/fq.gz'
		or
		quit
	"""
...
def check_phred(arg):
	...
	if phred != None :
		...
		if not (phred == 33 or phred == 64):
			sys.exit("Error : Value for the option 'phred' can only be '33' or \
'64'.")
		else :
			return 1
...
':'")
		...
		if not illum[2].isdigit():
			sys.exit("Error: Value for palindrome clip threshold must be an \
integer")

		# check that an integer is entered for single clip threshold
		if not illum[3].isdigit():
			sys.exit("Error: Value for simple clip threshold must be an \
integer")
...
def check_slidingwindow(arg):
 ...
 ':'")
		...
		if not slidw[0].isdigit():
			sys.exit("Error : Value for window-size (sliding-window) must be an\
 integer")
	
		# check that an integer is entered for required quality
		if not slidw[1].isdigit():
			sys.exit("Error : Value for required-quality (sliding-window) must\
 be an integer")
...
':'")
		...
		if not maxinfo[0].isdigit():
			sys.exit("Error : Value for target-length (maxinfo) must be an \
integer")
		...
		if (nb < 0 or nb > 1):
			sys.exit("Error : Value for strictness must be a float between 0 \
and 1")
